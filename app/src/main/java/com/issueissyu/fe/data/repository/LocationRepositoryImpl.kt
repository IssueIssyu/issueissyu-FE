...
class LocationRepositoryImpl @Inject constructor(
    ...
    override suspend fun certifyUserLocation(lat: Double, lng: Double): Result<String> {
        return try {
            ...
            Result.failure(Exception(ApiErrorMapper.NETWORK_ERROR_MESSAGE, e))
        } catch (e: Exception) {
            Result.failure(apiErrorMapper.toException(e, "위치 인증에 실패했습니다."))
        }
    }

    private fun locationCertifyException(
        code: String?,
        message: String?,
    ): Exception {
        val fallbackMessage = when (code) {
            "LOCATION_400_1" -> "좌표 형식 또는 범위가 올바르지 않습니다."
            "LOCATION_404_1" -> "주소 조회 결과를 찾을 수 없습니다."
            "LOCATION_404_3" -> "법정동 코드 조회 결과를 찾을 수 없습니다."
            "LOCATION_502_2" -> "주소 변환 서비스 연결에 실패했습니다."
            "COMMON_500" -> "일시적인 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
            else -> "위치 인증에 실패했습니다."
        }
